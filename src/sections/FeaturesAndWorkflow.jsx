...
function FeaturesAndWorkflow() {
    ...
    return (
        <section className="py-16 px-4 relative overflow-hidden" id="features-workflow">
            {/* Background elements */}
            <div className="absolute inset-0 bg-gradient-to-b from-transparent via-lime-400/5 to-transparent"></div>
            <div className="absolute top-20 right-10 w-64 h-64 bg-lime-400/5 rounded-full blur-3xl"></div>
            <div className="absolute bottom-20 left-10 w-64 h-64 bg-lime-400/5 rounded-full blur-3xl"></div>
            ...
            <div className="max-w-6xl mx-auto relative">
                ...
                <div className="text-center mb-12">
                    ...
                    {/* Tab navigation */}
                    <div className="flex justify-center mt-8 space-x-2 border border-white/10 rounded-full p-1 bg-black/20 inline-flex">
                        <button
                            onClick={() => setActiveTab('features')}
                            className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${activeTab === 'features' ? 'bg-lime-400 text-black' : 'text-white/70 hover:text-white'}`}
                        >
                            Features
                        </button>
                        <button
                            onClick={() => setActiveTab('workflow')}
                            className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${activeTab === 'workflow' ? 'bg-lime-400 text-black' : 'text-white/70 hover:text-white'}`}
                        >
                            How It Works
                        </button>
                    </div>
                ...
                <div className="flex flex-wrap justify-center gap-2 mb-12">
                    {featuredFeatures.map((feature, index) => (
                        <div
                            key={index}
                            className="bg-neutral-900/50 border border-white/10 px-4 py-2 rounded-full text-sm inline-flex items-center gap-2 hover:border-lime-400/30 transition-all duration-300 group"
                        >
                            <div className="w-2 h-2 rounded-full bg-lime-400 group-hover:scale-125 transition-transform"></div>
                            {feature}
                        </div>
                    ))}
                </div>
                ...
                {activeTab === 'features' ? (
                    ...
                    <>
                        ...
                        <div className="mb-12 bg-neutral-900/80 border border-white/10 rounded-2xl p-8 backdrop-blur-sm">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    ...
                                    <h3 className="text-2xl font-semibold mb-4">Simplifying Your Job Search</h3>
